...
def create_document(data, doctype):
    try:
        ...
        frappe.log_error(title="mapping_details",message=mapping_details)
        frappe.log_error(title="mapping_details",message=len(data))
        ...
        frappe.log_error(title=f"create {doctype}",message=frappe.get_traceback())

def set_default_uoms(doc):
    doc.uoms = []
    doc.append("uoms",{
        "uom":"Nos",
        "conversion_factor":"1"
...
def create_address(data,customer_id):
    ...
    if data.get("customer_billadrline1") and data.get("bill_city_name") and data.get("bill_country_name"):
        billing_address_details = frappe.db.sql("""SELECT  dl.parent as 'parent'
    FROM 
        `tabDynamic Link` dl
    JOIN 
        `tabAddress` a
    ON 
        dl.link_name = a.name
    WHERE 
        dl.link_name = %s and is_primary_address=1""",customer_id,as_dict=1)
        if len(billing_address_details) >= 1:
            address_doc = frappe.get_doc("Address",billing_address_details[0].parent)
        else:
    ...
    if data.get("customer_shipadrline1") and data.get("ship_city_name") and data.get("ship_country_name"):
        shipping_address_details = frappe.db.sql("""SELECT  dl.parent as 'parent'
    FROM 
        `tabDynamic Link` dl
    JOIN 
        `tabAddress` a
    ON 
        dl.link_name = a.name
    WHERE 
        dl.link_name = %s and is_shipping_address=1""",customer_id,as_dict=1)
        if len(shipping_address_details) >= 1:
            address_doc = frappe.get_doc("Address",shipping_address_details[0].parent)
        else:
            address_doc = frappe.new_doc("Address")
        ...
        request_log = frappe.get_doc(
            {
                ...
                "output": res.text if res else None,
            }
        )
...
def delete_customer_data():
    ...
    frappe.db.sql("""DELETE `tabAddress` FROM `tabAddress`
INNER JOIN `tabDynamic Link` ON `tabAddress`.name = `tabDynamic Link`.parent
WHERE `tabDynamic Link`.link_doctype = 'Customer'""")
    frappe.db.sql("""DELETE from `tabDynamic Link` where link_doctype = 'Customer'""")